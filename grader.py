...
class Grader:
    ...
    def _score_correctness(self, history: List[Dict]) -> float:
        ...
        return max(0.1, min(0.9, round(sum(values) / len(values), 4)))
    ...
    def _score_efficiency(self, task_id: int, summary: Dict) -> float:
        ...
        if steps <= min_s:
            return 0.9
        return min(0.9, max(0.1, round(min_s / steps, 4)))
    ...
    def _score_quality(self, task_id: int, history: List[Dict], summary: Dict) -> float:
        ...
        if task_id == 1:
            ...
            quality = max(0.1, min(0.9, 1.0 - (bad_actions / max(1, len(history)))))
        ...
        return min(0.9, max(0.1, round(float(quality), 4)))
    ...
    def _score_risk(self, history: List[Dict], summary: Dict) -> float:
        ...
        score = max(0.1, min(0.9, 1.0 - final_risk - penalty))
